...
<body>
	...
	<div class="container">
		...
		<%
		// Retrieve the quantity from the request (it might be null)
		Integer quantityObj = (Integer) request.getAttribute("inputboxquantity");
		// Default to 0 if quantityObj is null
		...
		<div class="action-buttons">
			...
			%>
			<a
				href="Menu?restaurantId=<%=menu.getRestaurantId()%>&quantity=<%=quantity%>">
				<div class="addmore">ADD CART ITEMS</div>
			</a>
			<%
			} else {
			%>
			<a
				href="Menu?restaurantId=<%=menu.getRestaurantId()%>&quantity=<%=quantity%>">
				<div class="addmore">ADD MORE ITEMS</div>
			</a> <a
				href="Checkout?menuId=<%=menu.getMenuId()%>&restaurantId=<%=menu.getRestaurantId()%>">
